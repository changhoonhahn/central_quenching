...
class CenQue: 
        ...
        for i_m in range(mass_bins.nbins):              
            ...
            if mass_bin_n_sf > 0: 
                ...
                if kwargs['sfr'] == 'sfr_avg': 
                    ...
                    # SFR assignment based on randomly displacing galaxies from 
                    # the average SFR of SF-MS 
                    ...
                    # SFR assignment based on randomly assigning 
                    # amplitude, frequency, and phase 
                    
                    # Average SFR and 1sigma of SF-MS SFR
                    [sf_avg_sfr, sf_sig_sfr] = util.get_sfr_mstar_z_bestfit(
                            mass_bins.mass_mid[i_m], self.zsnap, Mrcut=18) 
                    ...
                    raise NotImplementedError('asdfkjasdflkj')
    ...
    def writeout(self, **kwargs): 
        ...
        if self.sfr is None: 
            # output columns 
            columns = ['halo_mass', 'mass', 'parent', 'child', 'ilk', 'snap_index']
            ...
            if kwargs['sfr'] == 'sfr_func':  
                # output columns 
                columns = ['halo_mass', 'mass', 'sfr', 'sfr_phase', 'sfr_freq', 'sfr_amp', 
                        'ssfr', 'gal_type', 'parent', 'child', 'ilk', 'snap_index']

            elif kwargs['sfr'] == 'sfr_avg': 
                # output columns 
                columns = ['halo_mass', 'mass', 'sfr', 'ssfr', 'gal_type', 'sfr_resid', 
                        'parent', 'child', 'ilk', 'snap_index']
            else: 
                raise NotImplementedError('asdfalkjasdf')
...
def EvolveCenQue(origin_nsnap, final_nsnap, mass_bin=None, silent=True, **kwargs): 
    ...
    * Quenching of SF galaxies now involve quenching-efold *and* overall SF MS SFR decrease
    ...
    for i_step in range(0, origin_nsnap - final_nsnap):    
        ...
        child_cq = CenQue() 
        child_cq.readin(nsnap=child_snap) # read in snapshot SHAM and Halo masses
        ...
        # --------------------------------------------------------------------------------
        # children inherit galaxy type and store parent SFR, mass, and halo mass
        ...
        if kwargs['sfr'] == 'sfr_avg': 
            ...
            raise NotImplementedError('asdfasdfasdfasdfasdflkj') 
        ...
        if kwargs['sfr'] == 'sfr_avg': 
            child_cq.writeout(nsnap=child_cq.nsnap, file_type='evol from '+str(origin_nsnap), 
                    columns = ['mass', 'sfr', 'ssfr', 'gal_type', 'tau', 'halo_mass', 'q_ssfr', 
...
def build_cenque_origin(i_snap = 13, **kwargs): 
    ''' Simple wrapper to assign SFR to Snapshot 13

    Notes
    -----
    * Evolution starts from this cenque

    '''
    
    mass_bins = util.simple_mass_bin()  # use simplest mass bins

    snap = CenQue()             # Construct CenQue class
    snap.AssignSFR(i_snap, **kwargs)    # Assign SFR
    snap.writeout(nsnap=i_snap, file_type='sf assign', **kwargs)    # write out 
...
if __name__=='__main__': 
    ...
    build_cenque_origin(sfr='sfr_avg')
    EvolveCenQue(13, 1, tau='linefit', tau_param=[-0.7, 0.4], sfr='sfr_avg', stellmass='sham')
