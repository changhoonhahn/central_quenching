pro read_group_catalog, mr_cut, groups=groups, prob=prob, galdata=galdata
; reads in the group catalog data made by Jeremy and writes fits files with appropriate columns
    if (mr_cut EQ 18) then mass_cut = '9.4'
    if (mr_cut EQ 19) then mass_cut = '9.8'
    if (mr_cut EQ 20) then mass_cut = '10.2'
    ...
    if keyword_set(galdata) then begin  
        ...
        readcol, file_dir+filename+file_ext, v1, id_gal, M_r, M_g, cz, Dn4000, H_delta_EW, SSFR, $
            stellmass, ra, dec, v_disp, sn, n_sersic, $
            format='A,UL,F,F,F,F,F,F,F,F,F,F,F,F'

        output = replicate({id_gal:0, M_r:0.0, M_g:0.0, cz:0.0, Dn4000:0.0, SSFR:0.0, H_delta_EW:0.0, $
