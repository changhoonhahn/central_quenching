...
\eeq
is fit to the SDSS DR7 group catalog central galaxies 
(\S \ref{sec:sdss}). $\alpha(\mathcal{M}_*)$, which dictates
...
%%% RefReport Major2.1
{\bf \color{dred}
We note that \cite{Tinker:2013aa} classifies galaxies as star-forming or 
quiescent based on a $(NUV-R) - (R-J)$ color-color cut from \cite{Bundy:2010aa}
rather than a classification based on SFR and stellar mass. 
In Appendix~\ref{app:sfq}
We confirm that $f_{\rm Q}$ calculated using an analogous 
$(NUV-R) - (R-J)$ color-color star-forming/quiescent classification
is consistent with $f_{\rm Q}$ calculated using a SFR-$\mathcal{M}_*$
classification. 
}
{\bf \color{darkgreen} 
Note that the quiescent fraction derived from (NUV-R)-(R-J) color-color SF/Q classification is
consistent with the quiescent fraction derived from SFR-M* SF/Q classifications at $z\sim 0$. 
}
%%%
...
For $f^\mathrm{model}_\mathrm{Q}$ rather than using the actual evolutionary 
stages of the simulation central galaxies, we measure it using the following 
$\mathrm{SFR} - \mathcal{M}_*$ cut. The $\mathrm{SFR} - \mathcal{M}_*$ cut
...
\subsection{Star-forming/Quiescent Classifications} \label{app:sfq}
In our parameterization of $\fqcen$ in Eqs.~\ref{eq:fq} and~\ref{eq:fqz0}, 
we derive the best-fit values for the parameters $A_0$ and $A_1$ by fitting 
the SDSS DR7 group catalog $\fqcen$. The SDSS DR7 group catalog 
$\fqcen$ is derived using a $\mathrm{SFR} - \mathcal{M}_*$ cut 
specified in Eq.~\ref{eq:sfr_cut}. However, the best-fit value for 
$\alpha(\mathcal{M}_*)$, which parameterizes the redshift dependence, 
is derived from fitting to \cite{Tinker:2013aa} $\fqcen$ measurements 
of the COSMOS survey. These $\fqcen$ measurements use 
$(NUV - R) - (R - J)$ color-color cuts described in \cite{Bundy:2010aa}
for the star-forming/quiescent classification. In this section, we 
demonstrate that the SDSS DR7 group catalog $\fqcen$, using a 
$\mathrm{SFR} - \mathcal{M}_*$ cut, are consistent with the 
\cite{Tinker:2013aa} $\fqcen$, using a $(NUV - R) - (R - J)$ 
color-color cut.

We construct a catalog with photometry from GALEX, SDSS, and 2MASS. 
The GALEX and SDSS photometry are obtained from the NASA-Sloan 
Atlas\footnote{http://www.nsatlas.org/}.
