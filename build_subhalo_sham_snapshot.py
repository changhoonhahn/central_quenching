...
# Reads in the snapshots using Andrew's subhalo_io, assigns M* using 
# Andrew's SHAM code then exports central galaxies with corresponding 
# child indices 

sub = subhalo_io.Treepm.read('subhalo',250,zis=range(1,16)) # read snapchats of galaxies from z~0.0502 to z~1.0833
sham.assign(sub, 'm.star', scat=0, dis_mf=0.0, zis=range(1,16)) # assign M* to subhalos using SHAM
snap_range = range(1,16)

for i in snap_range: 
    cen_index = utility.utility_catalog.indices_ilk(sub[i], ilk='cen') # indices of central galaxies for snapshot i 
    ...
    if (i==np.min(snap_range)):     # child index of subhalo
